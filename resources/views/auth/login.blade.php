...
<div class="max-w-md mx-auto sm:my-12 px-0 sm:px-4">
    ...
    <div class="text-center mb-8 px-4 sm:px-0">
        ...
        <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Welcome Back</h1>
        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Sign in to XenProfessional to join the discussion.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-none sm:rounded-[2rem] border-y sm:border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative">
        ...
        <div class="p-8 sm:p-10 space-y-6">
            ...
            <div class="relative flex items-center py-2">
                ...
                <span class="flex-shrink-0 mx-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Or sign in with email</span>
                <div class="flex-grow border-t border-slate-200 dark:border-slate-800"></div>
            </div>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <div class="space-y-2 text-left">
                    <div class="flex items-center justify-between ml-1">
                        <label for="password" class="text-[11px] font-black text-slate-700 dark:text-slate-350 uppercase tracking-widest">Password</label>
                        <a href="#" class="text-[10px] font-bold text-blue-600 dark:text-blue-400 hover:underline">Forgot?</a>
                    </div>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <span class="material-symbols-outlined text-slate-400 text-[18px]">lock</span>
                        </span>
                        <input type="password" id="password" name="password" class="w-full bg-slate-50/50 dark:bg-slate-950/20 border border-slate-200 dark:border-slate-800 rounded-2xl pl-11 pr-4 py-3.5 text-slate-800 dark:text-white text-sm font-semibold focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder:text-slate-400 dark:placeholder:text-slate-655 placeholder:font-medium shadow-inner shadow-slate-100/50 dark:shadow-none" placeholder="••••••••" required>
                    </div>
                    @error('password')
                        <p class="text-xs text-rose-500 mt-1.5 ml-1 font-bold">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center ml-1">
                    <input type="checkbox" id="remember" name="remember" class="w-4.5 h-4.5 rounded-lg bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-blue-600 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer transition-colors">
                    <label for="remember" class="text-[11px] text-slate-600 dark:text-slate-300 ml-2.5 font-bold cursor-pointer select-none">Remember me next time</label>
                </div>

                <!-- Submit Action -->
                <div class="pt-4">
                    <button type="submit" class="w-full relative group overflow-hidden bg-slate-900 dark:bg-slate-850 hover:bg-slate-800 text-sm font-bold text-white py-4 rounded-2xl shadow-lg shadow-slate-900/20 cursor-pointer transition-all">
        ...
        <!-- Footer Link -->
        <div class="bg-slate-50 dark:bg-slate-950/40 border-t border-slate-100 dark:border-slate-850 p-6 text-center">
            <p class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                Don't have an account? <a href="{{ route('register') }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 hover:underline">Register here</a>
            </p>
        </div>
    </div>
</div>
