    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left px-4 sm:px-0">
        <div>
            ...
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-2">
                <a href="{{ route('home') }}" class="hover:text-blue-606 dark:hover:text-blue-400 transition-colors">Forums</a>
                <span>/</span>
                <span class="text-blue-600 dark:text-blue-400 font-semibold">Search Results</span>
            </div>
            <!-- Title -->
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-950 dark:text-white tracking-tight flex items-center gap-3">
                <span class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-50 dark:bg-blue-950/20 flex items-center justify-center text-blue-600 dark:text-blue-400 border border-blue-150/60 dark:border-blue-900/30 shadow-sm">
                    <span class="material-symbols-outlined text-xl">search</span>
                </span>
                Search Results
            </h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed mt-1.5">
                @if($query)
                    Showing results for "<span class="font-bold text-slate-800 dark:text-slate-200">{{ $query }}</span>"
                @else
        ...
        <div>
            ...
            <form action="{{ route('search') }}" method="GET" class="relative">
                <input type="text" name="q" value="{{ request('q') }}" class="w-full sm:w-64 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold shadow-sm" placeholder="Search again...">
                <span class="material-symbols-outlined absolute left-3 top-2.5 text-slate-400 dark:text-slate-600 text-sm">search</span>
                ...
                <div class="px-4 py-3.5 sm:px-6 sm:py-4.5 flex items-center justify-between gap-4 hover:bg-slate-50/40 dark:hover:bg-slate-950/20 transition-colors">
                    ...
                    <div class="flex items-center gap-3.5 flex-grow min-w-0">
                        ...
                        <div class="space-y-0.5 text-left min-w-0 flex-grow">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/35 shadow-sm leading-none">
                            ...
                            <!-- Subtitle / Meta info (Super Compact on mobile) -->
                            <div class="flex items-center gap-1.5 text-[10px] sm:text-xs text-slate-450 dark:text-slate-500 font-bold flex-wrap leading-none">
                                <span>By</span>
                                <a href="{{ route('profile.show', $thread->user->name) }}" class="text-slate-650 dark:text-slate-350 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">{{ $thread->user->name }}</a>
                                <span>•</span>
                    ...
                    <div class="hidden md:flex items-center gap-16 flex-shrink-0">
                        ...
                        <div class="text-xs text-slate-500 dark:text-slate-400 font-medium w-44 text-right">
                            ...
                            @else
                                <span class="text-slate-450 dark:text-slate-655 font-bold block">No replies yet</span>
                            @endif
                        </div>
                    </div>
            ...
            @empty
                <div class="px-6 py-16 text-center">
                    <div class="w-14 h-14 bg-slate-50 dark:bg-slate-950/50 rounded-2xl flex items-center justify-center text-slate-450 dark:text-slate-650 mx-auto mb-4 border border-slate-200 dark:border-slate-850 shadow-inner">
                        <span class="material-symbols-outlined text-2xl animate-pulse">search_off</span>
                    </div>
                    <h3 class="font-bold text-slate-800 dark:text-slate-200 text-base mb-1">No Results Found</h3>
                    <p class="text-xs text-slate-450 dark:text-slate-505 max-w-sm mx-auto mb-4 font-medium">We couldn't find any threads matching your query. Try different keywords.</p>
                </div>
            @endforelse
        </div>
    </div>
